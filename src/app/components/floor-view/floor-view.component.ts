  ...
  <div class="bg-white rounded-xl shadow p-3 overflow-auto">
    <ng-container *ngIf="schema(); else noMap">
      <svg [attr.viewBox]="viewBox()" class="w-full h-[420px] border rounded">
        <rect x="0" y="0" [attr.width]="schema()!.width" [attr.height]="schema()!.height" fill="#f8fafc"></rect>
        <ng-container *ngFor="let r of schema()!.rooms">
          <rect [attr.x]="r.x" [attr.y]="r.y" [attr.width]="r.w" [attr.height]="r.h"
                ...
                stroke="#0F3F7A" stroke-width="1.5" rx="6" ry="6" />
          <text [attr.x]="r.x + 8" [attr.y]="r.y + 18" font-size="12" fill="#0F172A">{{ r.name }}</text>
        </ng-container>
      </svg>
    </ng-container>
    <ng-template #noMap><div class="text-gray-500">No hay mapa para este piso.</div></ng-template>
  </div>
  `
})
export class FloorViewComponent implements OnChanges {
  ...
  ngOnChanges(ch: SimpleChanges): void {
    if (('deptId' in ch || 'floor' in ch) && this.deptId && this.floor) {
      this.data.getFloor$(this.deptId, this.floor).subscribe(s => this.schema.set(s));
    }
  }
}
